...
public class MeditatieController extends ConnectionController{
    public static void AddNew(int id_elev,int id_prof){
        int last_entry=0;
        try{
            pst=conn.prepareStatement("select id from meditatii order by id desc limit 1");
            rs=pst.executeQuery();
            while(rs.next()){
        ...
        try {
            pst=conn.prepareStatement("insert into meditatii(room,id_elev,id_profesor,status) values(?,?,?,?)");
            pst.setInt(1,last_entry);
